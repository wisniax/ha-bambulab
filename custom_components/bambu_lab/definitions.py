...
PRINTER_SENSORS: tuple[BambuLabSensorEntityDescription, ...] = (
    ...
    BambuLabSensorEntityDescription(
        key="gcode_model_printing_time",
        translation_key="gcode_model_printing_time",
        icon="mdi:clock-outline",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_model_printing_time != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_model_printing_time,
    ),
    BambuLabSensorEntityDescription(
        key="gcode_total_estimated_time",
        translation_key="gcode_total_estimated_time",
        icon="mdi:clock-outline",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_estimated_time != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_estimated_time,
    ),
    BambuLabSensorEntityDescription(
        key="gcode_total_filament_length",
        translation_key="gcode_total_filament_length",
        icon="mdi:ruler",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_filament_length != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_filament_length,
    ),
    BambuLabSensorEntityDescription(
        key="gcode_total_filament_volume",
        translation_key="gcode_total_filament_volume",
        icon="mdi:database",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_filament_volume != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_filament_volume,
    ),
    BambuLabSensorEntityDescription(
        key="gcode_total_filament_weight",
        translation_key="gcode_total_filament_weight",
        icon="mdi:weight-gram",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_total_filament_weight != "",
    ...
    BambuLabSensorEntityDescription(
        key="gcode_user_on_list",
        translation_key="gcode_user_on_list",
        icon="mdi:account-details",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_user_on_list != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_user_on_list,
    ),
    BambuLabSensorEntityDescription(
        key="gcode_user_allowed_prefix",
        translation_key="gcode_user_allowed_prefix",
        icon="mdi:account-filter",
        available_fn=lambda self: self.coordinator.get_model().print_job.gcode_user_allowed_prefix != "",
        value_fn=lambda self: self.coordinator.get_model().print_job.gcode_user_allowed_prefix,
    ),
    BambuLabSensorEntityDescription(
        key="subtask_name",
